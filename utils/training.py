...
def train_1(train_dataloader, val_dataloader, model, use_history=False,
        ...
        for batch_idx, data in enumerate(train_dataloader, 0):
            ...
            if steps_save is not None:
                ...
                if batch_idx % steps_save == steps_save-1:
                    torch.cuda.empty_cache()
                    torch.save(model.state_dict(), f'{folder_name}\\{model_name}.pt')
                    torch.save(optimizer.state_dict(), f'{folder_name}\\{model_name}_optimizer')
